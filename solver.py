...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader, config,target_label):
    ...
    def build_model(self):
        ...
        #    self.G = nn.DataParallel(self.G, device_ids=[0, 1, 2, 3])
        #    self.D = nn.DataParallel(self.D, device_ids=[0, 1, 2, 3])
    ...
    def create_labels(self, c_org, c_dim=5, dataset='CelebA', selected_attrs=None):
        ...
        for i in range(c_dim):
            ...
            #print(c_trg_list)
        return c_trg_list   #生成的c_trg_list是一个包含5个[16,5]的list。 生成的list就是后五个图对应的目标标签（[‘Black_Hair’, ‘Blond_Hair’, ‘Brown_Hair’, ‘Male’, ‘Young’]）
    ...
    def train(self):
        ...
        # Fetch fixed inputs for debugging.取一个Batch的固定图片，方便后面来看训练效果
    ...
    def train_multi(self):
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                d_loss_cls = self.classification_loss(out_cls, label_org, dataset)#衡量真实标签与标签估计的差距

                # Compute loss with fake images.
                x_fake = self.G(x_real, c_trg)#输入一个batch的真实图片和目标标签，生成假的图
                out_src, _ = self.D(x_fake.detach())
                d_loss_fake = torch.mean(out_src)#判定越接近为假，损失越大
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                str_target_label=self.target_label
                ndarray = np.zeros((1,self.c_dim))
                for kk in range(self.c_dim):
                    ndarray[0][kk]=int(str_target_label[kk])

                c_trg_list=torch.FloatTensor(ndarray)

                x_fake_list =[self.denorm(x_real)]

                # Translate images.
                x_fake=self.G(x_real, c_trg_list)
                x_fake_list.append(self.denorm(x_fake))

                save_image(self.denorm(x_fake.data.cpu()), self.result_dir, nrow=1, padding=0)

                dct=cv2.imread(self.result_dir)
                cv2.fastNlMeansDenoisingColored(dct, dct)

                cv2.imwrite(self.result_dir, dct)
                loader = transforms.Compose([transforms.ToTensor()])
                rec_img = Image.open(self.result_dir).convert('RGB')
                rec_img = loader(rec_img).unsqueeze(0)
                rec_img=rec_img.to(self.device, torch.float)
                print(rec_img.size())
                x_fake_list.append(rec_img)

                x_concat = torch.cat(x_fake_list, dim=3)
                x_concat = x_concat.data.cpu()
